...
for char in Chars
  ...
  for {c, x, y, w, h} in comp
    ...
    if fs.exists-sync "json/#ref-hex.json"
      id++
      console.log "INSERT INTO refs VALUES (#id, '#char', #part, '#c', '#c', 0, 0, #x, #y, #w, #h);"
    if comp-chars?len
      for whole, offset of comp-chars.src
        id++
        console.log "INSERT INTO refs VALUES (#id, '#char', #part, '#c', '#whole', #offset, #{ comp-chars.len }, #x, #y, #w, #h);"
