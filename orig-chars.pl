...
#   psql chars -P t -c "select st_asgeojson(st_rotate(ST_FlipCoordinates(st_translate(st_scale(st_collect(st_collect(outlines), st_collect(tracks)), 0.001, 0.001), 24.4416976, 121.4509512)),pi()/2*3,121.4509512,24.4416976)) from strokes where ch ='萌';" > moe.geojson
use 5.12.0;
use File::Slurp;
use JSON::XS;
binmode STDOUT, ':utf8';
